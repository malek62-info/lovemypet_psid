...
import { Bot, Dog, ChartLine, Home, Menu, X } from 'lucide-react'
...
const Navbar = () => {
    ...
    const navas = [
        ...
        { href: "/dashboard", label: "Tableau de bord", icon: <ChartLine  className="w-5 h-5 " /> },
            ...
            {navas.map(({ href, label, icon }) => {
                ...
                return (
                    <Link
                        to={href}
                        key={href}
                        className={`${classNames} btn ${isActive ? 'btn-primary' : ''} flex items-center gap-2`}
                    >
